1. 从“在浏览器输入域名”到“页面静态资源完全加载”的整个流程
    ...
    *******************************************************
    ...
    - udp无连接的，面向字节流，支持多对多
    - tcp 面向连接的，传输可靠，有序，三次握手建立连接，只能点对点、

      - TCP3次握手过程
        1. 第一次握手：主机A通过向主机B发送一个含有同步序列号的标志位的数据段给主机B，向主机B请求建立连接，通过这个数据段，主机A告诉主机B两件事：我想要和你通信；你可以用哪个序列号作为起始数据段来回应我
        2. 第二次握手：主机B收到主机A的请求后，用一个带有确认应答ACK和同步序列号SYN标志位的数据段响应主机A，也告诉主机A两件事：我已经收到你的请求了，你可以传输数据了；你要用哪个序列号作为起始字段来回应我
        3. 第三次握手：主机A收到这个数据段之后，再发送一个确认应答，确认已经收到主机B的数据段：我已经收到回复，现在我要开始实际传输数据了。这样三次握手就完成了，主机AB之间可以传输数据了。

        - 3次握手的特点：没有应用层的数据，SYN这个标志位只有在TCP建立连接时才会被置为1，握手完成后SYN标志位被置为0
      
      TCP建立连接要进行3次握手，而断开连接要进行4次
      1. 第一次：当主机A完成数据传输之后，将控制位FIN置1，提出停止TCP连接的请求；
      2. 第二次：主机B收到FIN后对其作出响应，确认这一方向上的TCP连接将关闭，将ACK置为1
      3. 第三次：B端再提出反方向的关闭请求将FIN置为1
      4. 第四次： 主机A对主机B的请求进行确认，将ACK置为1，双方向的关闭结束


      ping命令使用IP和网络控制信息协议ICMP
